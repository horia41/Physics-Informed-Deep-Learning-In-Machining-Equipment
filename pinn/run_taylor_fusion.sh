...
#
# NOTE on --base-exp t3_gated_top25_md30:
#   The gated-fusion grid (t3_gated_top25_md30) lives in the V2 trainer
#   (train_vision_sensorV2.py), which ships in pinn_aircuts/ and
#   vision-sensor/improve_attempt/. This pinn/ trainer resolves its base
#   experiments from vision-sensor/train_vision_sensor.py (the NON-gated grid:
#   vision_only_647, early/intermediate/late × raw25/top25/all40). To run the
#   gated base, use pinn_aircuts/ (see pinn_aircuts/README.md), or pass
#   a --base-exp that exists in the non-gated grid (e.g. intermediate_top25).
#
# Needs taylor_constants.json (fit once on the login node — see fit_taylor.py).
