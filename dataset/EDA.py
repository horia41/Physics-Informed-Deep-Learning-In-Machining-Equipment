...
WEAR_TYPE_ORDER = ["flank_wear", "adhesion", "flank_wear+adhesion"]
...
SPLIT_COLOR = {"train": "#4C72B0", "val": "#55A868",
...
def analyse_integrity(df: pd.DataFrame, data_dir: Path, out_dir: Path) -> None:
    ...
    save(fig, out_dir, "01_data_availability")
...
def analyse_labels(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    for mat in ["CK45", "RVS 304"]:
        ...
        ax2.hist(sub, bins=30, alpha=0.65,
                 color=PALETTE[mat], label=f"{mat} (n={len(sub)})", edgecolor="white")
    ...
    for patch, wt in zip(bp["boxes"], WEAR_TYPE_ORDER):
        patch.set_facecolor(PALETTE.get(wt, "grey"))
        patch.set_alpha(0.7)
    ax3.set_xticks(range(1, len(WEAR_TYPE_ORDER)+1))
    ax3.set_xticklabels([wt.replace("_", "\n") for wt in WEAR_TYPE_ORDER], fontsize=8)
    ...
    save(fig, out_dir, "02_wear_distribution")
    ...
    pivot = (valid.groupby(["Set", "type"])
                  ...
                  .reindex(columns=WEAR_TYPE_ORDER, fill_value=0))
    bottom = np.zeros(len(pivot))
    for wt in WEAR_TYPE_ORDER:
        if wt in pivot.columns:
            ax.bar(pivot.index, pivot[wt], bottom=bottom,
                   color=PALETTE.get(wt, "grey"), label=wt, alpha=0.85)
    ...
    for set_num in pivot.index:
        ...
        ax.axvspan(set_num - 0.4, set_num + 0.4,
                   color=SPLIT_COLOR[split], alpha=0.08, zorder=0)

    # Material separator
    ax.axvline(11.5, color="black", linestyle=":", linewidth=1.5, label="CK45 | RVS 304")

    # Legend
    type_patches = [mpatches.Patch(color=PALETTE[wt], label=wt) for wt in WEAR_TYPE_ORDER]
    split_patches = [mpatches.Patch(color=SPLIT_COLOR[s], alpha=0.4, label=f"split:{s}")
                     for s in SPLIT_COLOR]
    ...
    save(fig, out_dir, "03_wear_type_per_set")
...
def analyse_trajectories(df: pd.DataFrame, out_dir: Path) -> None:
        ...
        for wt in WEAR_TYPE_ORDER:
            ...
            if len(wt_sub):
                ax.scatter(wt_sub["ImageID"], wt_sub["wear"],
                           c=PALETTE.get(wt, "grey"), s=18, alpha=0.8,
                           zorder=3, label=wt)
    ...
    legend_elements = [Line2D([0],[0], marker="o", color="w",
                               markerfacecolor=PALETTE[wt], label=wt, markersize=7)
                       for wt in WEAR_TYPE_ORDER]
    ...
    save(fig, out_dir, "04_wear_trajectories", dpi=150)
    ...
    save(fig, out_dir, "05_material_comparison_trajectories")
...
def analyse_cutting_params(df: pd.DataFrame, sets_df: pd.DataFrame,
    ...
    for ax, (param, label) in zip(axes, params):
        ...
        for mat in ["CK45", "RVS 304"]:
            ...
            ax.scatter(ms[param], ms["max_wear"],
                       color=PALETTE[mat], s=80, alpha=0.8,
    ...
    fig.suptitle("Cutting parameters vs maximum wear reached per set",
                 fontsize=12, fontweight="bold")
    save(fig, out_dir, "06_cutting_params_vs_wear")
...
def analyse_sensors(df: pd.DataFrame, data_dir: Path, out_dir: Path) -> None:
    ...
    fig.suptitle("Raw sensor signals — early / mid / late in tool life (Set 5)",
                 fontsize=12, fontweight="bold")
    save(fig, out_dir, "07_sensor_signals_raw", dpi=120)
    ...
    for idx, (ch, ax) in enumerate(zip(channels, axes_flat)):
        ...
        for mat in ["CK45", "RVS 304"]:
            ...
            ax.scatter(ms["wear"], ms[rms_col],
                       color=PALETTE[mat], alpha=0.7, s=40,
    ...
    fig.suptitle("Sensor RMS vs wear level (by material)",
                 fontsize=12, fontweight="bold")
    save(fig, out_dir, "08_sensor_rms_vs_wear")
...
def _plot_sensor_placeholder(out_dir: Path) -> None:
    ...
    ax.set_axis_off()
    save(fig, out_dir, "07_sensor_signals_placeholder")
...
def analyse_images(df: pd.DataFrame, data_dir: Path, out_dir: Path) -> None:
    ...
    fig.suptitle("Sample images at different wear stages",
                 fontsize=13, fontweight="bold")
    save(fig, out_dir, "09_image_samples", dpi=120)
...
def _plot_image_placeholder(out_dir: Path) -> None:
    ...
    ax.text(0.5, 0.5,
            ...
            transform=ax.transAxes)
    ax.set_axis_off()
    save(fig, out_dir, "09_image_samples_placeholder")
...
def analyse_modelling(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax = axes[0]
    for split_label, color in SPLIT_COLOR.items():
    ...
    bars = ax.bar(split_order, [f*100 for f in adhes_fracs],
                  color=[SPLIT_COLOR[s] for s in split_order], alpha=0.8,
    ...
    save(fig, out_dir, "10_split_analysis")
    ...
    # ── Key takeaways ─────────────────────────────────────────────────────────
  #   subsection("Key takeaways for modelling")
  #   print("""
  # 1. SAMPLING MISMATCH
  #    Each sensor CSV = ~tens of thousands of rows for a single image measurement.
  #    → Need feature engineering (FFT, RMS, Kurtosis) to create per-pass summary vectors.
  #
  # 2. SYNC ERRORS
  #    Some rows have image but no sensor (or vice versa).
  #    → Dataset class must handle missing modalities gracefully.
  #
  # 3. ADHESION IS THE HARD CASE
  #    Regression MAE jumps from ~14 µm (flank) to ~91 µm (flank+adhesion) in the paper.
  #    → Physics constraint (Taylor) should act as a corrective prior here.
  #
  # 4. SET 17 HAS z=2 (TWO INSERTS)
  #    Taylor's equation uses z (number of teeth). Set 17 behaves differently.
  #    → Either fit Taylor's C separately, or treat Set 17 as a separate domain.
  #
  # 5. SET 1 HAS UNKNOWN CUTTING PARAMETERS
  #    Cannot apply Taylor's equation to Set 1 without imputing Vc.
  #    → Either exclude from PINN loss or impute from similar sets (2–4 range).
  #
  # 6. SET 6 HAS VARIABLE Vf
  #    Feed rate changed mid-run → Taylor assumption of constant V is violated.
  #    → Use with caution in physics loss; consider excluding from Taylor computation.
  #
  # 7. TRAIN/VAL/TEST SPLIT IS SET-LEVEL (no data leakage)
  #    Never mix rows from the same set across splits.
  #    → Always split at the Set level, not at the sample level.
  #
  # 8. RVS 304 (Sets 12–17) PRODUCES MORE ADHESION
  #    Including them in training helps generalisation but makes the task harder.
