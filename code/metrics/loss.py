...
class TaylorPhysicsLossV1(nn.Module):
    def __init__(
        self,
    ...
    def _build_lookup_tables(self) -> None:
        ...
        mat_code = np.full(max_set, MAT_CODE_UNKNOWN, dtype=np.int64)
        ...
        for s, rec in self.set_recs.items():
            ...
            mat_code[s] = material_to_code(rec["material"])
            valid[s]    = True
    ...
    def forward(
        ...
        elif self.apply_to == "rvs":  mat_ok = mat_code == MAT_CODE_RVS
        else:                          mat_ok = mat_code == MAT_CODE_CK45
...
class TaylorPhysicsLossV2(nn.Module):
    def __init__(
        self,
        constants_path:   str | Path | None = None,
        payload:          Optional[dict] = None,
...
class TaylorPhysicsLossV3(nn.Module):
    def __init__(
        self,
        constants_path:   str | Path | None = None,
        payload:          Optional[dict] = None,
