...
from experiments.utils import predictions_to_um
...
from experiments.vision_sensor_experiment import build_loaders, evaluate
from model.vision_sensor.multimodal_model import MATWIMultimodalModel
from metrics.loss.taylor_physics_loss import TaylorPhysicsLoss
from metrics.fmt import fmt_metrics



def _train_one_epoch_taylor_vision_sensor(model, loader, optimizer, data_criterion, physics_loss,
...
def run_experiment_vision_sensor(base_cfg, args, device, out_dir: Path, name: str) -> None:
    print(f"\n{'='*80}\n  STAGE 3 FUSION: {name}")
    ...
    for epoch in range(1, base_cfg.epochs + 1):
        t0 = time.time()
        tr = _train_one_epoch_taylor_vision_sensor(model, loaders["train"], optimizer, data_criterion,
