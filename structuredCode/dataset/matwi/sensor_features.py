...
import pywt
from structuredCode.dataset.matwi.dataset_multimodal import MATWIMultimodalDataset
from typing import Optional

from constants import WAVELET_BASELINE_N, WAVELET_LEVEL, WAVELET_MIN_ACTIVE, WAVELET_NAME, WAVELET_TRIM, WAVELET_Z_THRESH, N_SENSOR_FEATURES, SENSOR_CHANNELS, SENSOR_FS, N_WINDOWS, BASELINE_K, SPLIT_SETS, CUTTING_FEATURE_NAMES
...
def _wavelet_cut_window(df: pd.DataFrame) -> tuple[int, int]:
    """
...
def _find_cut_segments(df: pd.DataFrame) -> list[tuple[int, int]]:
...
def extract_sensor_features(sensor_path: Path,
    ...
    if gate_aircuts:
        start, end = _wavelet_cut_window(df)
...
def extract_sensor_features_v3(sensor_path: Path) -> tuple[np.ndarray, float]:
    """
...
def extract_features_v4(sensor_path: Path) -> tuple[np.ndarray, int]:
    ...
    feats, _ = extract_sensor_features_v3(sensor_path)
    return feats, 1
...
def extract_all_features(
    ...
    for i in range(n):
        ...
        X_v1[i] = extract_sensor_features(path)
        X_v2[i] = extract_sensor_features_v2(path)
...
def extract_v3_features(
    ...
    for i in range(n):
        path = data_dir / str(df.iloc[i]["SensorFile"])
        X_v3[i], frac[i] = extract_sensor_features_v3(path)
        if (i + 1) % 100 == 0 or i == n - 1:
            elapsed = time.time() - t0
            rate    = (i + 1) / max(elapsed, 1e-6)
