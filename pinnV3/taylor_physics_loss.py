    ...
    def forward(
        ...
        for i in range(pred.shape[0]):
            ...
            if self.use_taylor_slope and rec.get("taylor_slope_um_per_pass"):
                ...
                # Live multi-variable backup calculator. Guard against a degenerate
                # fit (n ~ 0, e.g. CK45 with constant Vc) which would divide by zero
                # and produce nan losses; fall back to the observed per-set slope.
                ...
                if abs(n_val) < 1e-6:
                    ...
                    t_life = np.exp(log_T)
                    if np.isfinite(t_life) and t_life > 0:
